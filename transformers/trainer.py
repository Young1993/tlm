    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch, ignore_keys_for_eval):
        ...
        if self.control.should_save:
            ...
            if 'eval_accuracy' in metrics:
                if metrics['eval_accuracy'] > self.eval_accuracy:
                    self.eval_accuracy = metrics['eval_accuracy']
                    self._save_checkpoint(model, trial, metrics=metrics)
            elif 'eval_matthews_correlation' in metrics:
                if metrics['eval_matthews_correlation'] > self.eval_accuracy:
                    self.eval_accuracy = metrics['eval_matthews_correlation']
                    self._save_checkpoint(model, trial, metrics=metrics)
            elif 'eval_pearson' in metrics:
                if metrics['eval_pearson'] > self.eval_accuracy:
                    self.eval_accuracy = metrics['eval_pearson']
                    self._save_checkpoint(model, trial, metrics=metrics)
            else:
                if metrics['eval_loss'] < self.eval_loss:
